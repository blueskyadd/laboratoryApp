<template>
    <div class="overdueEquilpment body_main">
        <Search  @searchList='searchList'/>
        <main-list  @loadmore='loadmore' ref="mainList"  :islistData='islistData'>
            <template>
                <li   v-for="(item, index) in Gauger_outmeteringList" :key="index" >
...
export default {
    ...
    data(){
        return{
            Gauger_outmeteringList: [],
            searchText:'',
            islistData: false,
        }
    },
    mounted(){
        this.$emit('setHeaderShow', {blod:true,title:'已过计量期设备数'});
        this.getGauger_outmeteringList(1,'',12);
    },
    methods:{
        getGauger_outmeteringList(pagenumber,searchText,page_size, success, error,searchCallback){
            this.$vux.loading.show();
            this.$http.get( pagenumber == 1 ? this.$conf.env.getGauger_outmeteringList + '?search=' + searchText + '&page_size=' + page_size: this.$conf.env.getGauger_outmeteringList + '?search=' + searchText + '&p=' + pagenumber+ '&page_size=' + page_size).then( res =>{
                if(!searchText){
                    this.Gauger_outmeteringList = pagenumber == 1 ? res.data.results: this.Gauger_outmeteringList.concat(res.data.results); 
                }else{
                ...
                this.islistData = this.Gauger_outmeteringList.length>0?false:true;
        ...
        loadmore(pagenumber,success, error){
            this.getGauger_outmeteringList(pagenumber,this.searchText,12, ()=>{success();},()=>{ error();});
        },
        //搜索
        searchList(searchtest){
            this.searchText = searchtest;
            this.getGauger_outmeteringList(1,searchtest,500,()=>{},()=>{},(data)=>{
                this.Gauger_outmeteringList = data;
